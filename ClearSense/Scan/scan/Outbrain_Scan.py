...
Scan_Name.send_keys("Outbrain_scan")
time.sleep(3)
...
Description.send_keys("This scan is regarding Outbrain without Custom Domain")
time.sleep(10)
...
Tag.send_keys("Outbrain-Scan, #Sportsphere website")
time.sleep(10)

#Select Redirect link Scan type 
...
#Select Outbrain Scan type
try:
    # Wait for the element to be present and visible
    Outbrain_button = WebDriverWait(driver, 20).until(
        EC.visibility_of_element_located((By.XPATH, "/html/body/p-dynamicdialog/div/div/div[2]/app-create-scan/div/app-scan-information/form/div[1]/div[5]/div[3]/div[3]/p-radiobutton/div/div[2]"))
    )

    # Scroll into view
    driver.execute_script("arguments[0].scrollIntoView(true);", Outbrain_button)

    # Click using ActionChains
    actions = ActionChains(driver)
    actions.move_to_element(Outbrain_button).click().perform()
    print("Radio button clicked successfully.")
except Exception as e:
    print("Error interacting with the radio button:", e)
...
try:
    button_icon = WebDriverWait(driver, 10).until(
        EC.element_to_be_clickable((By.XPATH, "/html/body/p-dynamicdialog/div/div/div[2]/app-send-email/div[1]/button/i"))
    )
    button_icon.click()
except Exception as e:
    print("code didnot copied", e)
